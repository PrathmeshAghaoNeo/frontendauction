...
export class ManageUserComponent implements OnInit {
  ...
  getCountryName(countryId: number | undefined): string {
    return (
      this.countries.find((c) => c.countryId === countryId)?.countryName ||
      'Unknown'
    );
  }
  ...
  get filteredUsers(): UserView[] {
    ...
    return this.users.filter((user) => {
      ...
      const matchesStatus =
        statusFilter === 0 || user.statusId === statusFilter;
  ...
  exportToExcel(): void {
    ...
    const exportData = this.filteredUsers.map((u) => ({
      ...
      'Email': u.email,
      'Gender': u.gender || '-',
      'Deposit Amount': u.deposit,
      'Mobile Number ': u.mobileNumber,
      'Company Number': u.companyNumber,
      'Company Name': u.companyName,
      'Country Name': this.getCountryName(u.countryId),
      'Status': this.getStatusName(u.statusId),
      'Role': this.getRoleName(u.roleId),
      'Chat Enabled': u.chatEnabled,
    }));
